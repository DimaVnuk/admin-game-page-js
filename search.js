...
okGoogle.addEventListener("input", (event) => {
    ...
    const filteredGame = checkboxFilteredItems(searchedGame, objOfFilteredProperties);
  ...
  checkboxFilterCollection.forEach(item => {
    item.addEventListener('change', event => {
        ...
        if (event.target.checked) {
            ...
            const filteredGames = checkboxFilteredItems(searchGame, objOfFilteredProperties);
  ...
  function checkboxFilteredItems(arr, value) {
    ...
    if (!ArrayWithFiltered.length && !checkedProp)
    return arr;
